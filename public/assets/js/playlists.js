function addToPlaylistDialog(idTracks) {
	$('#dialog-addToPlaylist #playlistcombo').load(baseUrl + 'playlist/playlists/0',
		function() {
...
function addToPlaylist(idTracks, idPlaylist, callback) {
	$.post(baseUrl + 'playlist/addPlaylistItems',
		{
...
function addToNewPlaylist(idTracks, playlistName, callback) {
	$.post(baseUrl + 'playlist/createSimple', { playlistName: playlistName }, function(data) {
		if(validateSession(data)) {
			var json = $.parseJSON(data);
			if(json.success) {
...
function addTracksToCurrentPlaylist(idTracks) {
	$.get(baseUrl + 'playlist/playlistItemsForIdTracks/' + encodeURIComponent(idTracks), function(data) {
