<div class="container_12">
	<div class="grid_12">
		...
		<p><a href="<?= base_url() . "edit_track/create/{$idGame}" ?>" class="btn btn-default">Create</a></p>
		<p><a href="<?= base_url() . "game/index/{$idGame}" ?>">Go to game page</a></p>
	</div>
</div>
...
<?php $b = TRUE; foreach($tracks as $track): ?>
	<div style="<?php if($b = !$b): ?> background-color: #dddddd;<?php endif; ?>" class="container_16">
		...
		<div class="grid_1">
			<a class="btn btn-xs btn-default" href="<?=base_url()?>edit_track/index/<?=$track->idTrack?>">Edit</a>
		</div>
		<div class="grid_1">
			<?= form_open(base_url() . "tracks_dashboard/delete/{$track->idGame}", array('id' => "delete_{$track->idTrack}")) ?>
				<input type="hidden" name="id" value="<?=$track->idTrack?>" />
