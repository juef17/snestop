...
<?php else: ?>
	<?php $b = TRUE; date_default_timezone_set("America/New_York"); foreach($tracks as $track): ?>
		...
		<div style="display: none; padding-top: 15px;" id="dialog-details_<?=$track->idTrack?>" title="<?=$game->titleEng . ' - ' . $track->title?>">
			...
			<?php elseif($loggedUser && !$track->isScreenshotSet):?><a href="<?=base_url()?>request_screenshot_track/index/<?=$track->idTrack?>"><?php endif;?>
			...
			<p id="shareTrack_<?=$track->idTrack?>" style="display: none"><input type="text" style="cursor: text" class="form-control" onClick="this.select();" value="<?=base_url()?>game/index/<?=$game->idGame?>/<?=$track->idTrack?>" readonly /></p>
			<h3>Reviews</h3>
			<?php if($loggedUser):?>
				<a href="<?=base_url()?>request_review/index/<?=$track->idTrack?>">Write a review</a>
			<?php else:?>
