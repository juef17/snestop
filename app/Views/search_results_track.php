...
<?php else: ?>
	...
	<?php $b = TRUE; foreach($tracks as $track): ?>
		<div style="border-bottom: 1px; <?php if($b = !$b): ?> background-color: #dddddd;<?php endif; ?>" class="container_16">
			...
			<?php endif; ?>
			<a class="grid_4" href="<?=base_url()?>game/index/<?=$track->idGame?>/<?=$track->idTrack?>"><?=$track->title?></a>
			<a class="grid_4" href="<?=base_url()?>game/index/<?=$track->idGame?>"><?=$track->gameTitleEng?></a>
		</div>
	<?php endforeach; ?>
<?php endif; ?>
...
<?php
	function thumbnail($id) {
		...
		$thumb = new Imagick(FCPATH . "assets/images/screenshots/track/{$id}.png");
