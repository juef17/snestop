...
<?php $b = TRUE; foreach($requests as $request): ?>
	<div style="<?php if($b = !$b): ?> background-color: #dddddd;<?php endif; ?>" class="container_12">
		...
		<div class="grid_1">
			<a href="<?=base_url()?>user_profile/index/<?=$request->userName?>"><?=$request->userName?></a>
		</div>
		<div class="grid_5">
			<a href="<?=$request->URL?>"><?=$request->URL?></a>
		</div>
		<div class="grid_1">
			<?= form_open(base_url() . 'communities_dashboard/deleteRequest', array('id' => 'deleteRequest_' . $request->idCommunityRequest)) ?>
...
<div class="container_12">
	<?= form_open(base_url() . 'communities_dashboard/add', array('id' => 'addCommunity')) ?>
		<div class="grid_2 columnheader">
...
<?php $b = FALSE; foreach($communities as $community): ?>
	<div style="<?php if($b = !$b): ?> background-color: #dddddd;<?php endif; ?>" class="container_12">
		<?= form_open(base_url() . 'communities_dashboard/update', array('id' => 'update_' . $community['idCommunity'])) ?>
		...
		<div class="grid_1">
			<?= form_open(base_url() . 'communities_dashboard/delete', array('id' => 'delete_' . $community['idCommunity'])) ?>
				<input type="hidden" name="id" value="<?= $community['idCommunity'] ?>" />
