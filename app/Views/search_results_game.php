		...
		if(count($games) > 0) {
			...
			foreach($menuItems as $p) {
				if($p == $currentPage || $p === '#' && $currentPage === 'numbers')
					...
					$menu .= '<a href="' . base_url() . 'search/browse/0/' . ($p == '#' ? 'numbers' : $p) . '">' . $p . '</a>&nbsp;&nbsp;';
			}
		}
?>
...
<?php else: ?>
	...
	<?php $b = TRUE; foreach($games as $game): ?>
		<div style="border-bottom: 1px; <?php if($b = !$b): ?> background-color: #dddddd;<?php endif; ?>" class="container_16">
			...
			<?php endif; ?>
			<a class="grid_6" href="<?=base_url()?>game/index/<?=$game->idGame?>"><?=$game->titleEng?></a>
			<a class="grid_6" href="<?=base_url()?>game/index/<?=$game->idGame?>"><?=$game->titleJap?></a>
		</div>
	<?php endforeach; ?>
<?php endif; ?>
...
<?php
	function thumbnail($id) {
		// TODO j'avais un problème sur mon serveur, fopen voulait rien savoir de localhost, j'savais pas trop comment le régler fait que j'ai été lâche:
		//$thumb = new Imagick(asset_url() . "images/screenshots/game/{$id}.png");
		//$thumb = new Imagick("https://snestop.jerther.com/snestop/site/assets/" . "images/screenshots/game/{$id}.png");
		$thumb = new Imagick(FCPATH . "assets/images/screenshots/game/{$id}.png");
