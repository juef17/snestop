...
<?php else: ?>
	...
	<?php $b = TRUE; foreach($reviews as $review): ?>
		<div style="<?php if($b = !$b): ?> background-color: #dddddd;<?php endif; ?>" class="container_12">
			...
			<p class="grid_2"><a href="<?=base_url()?>user_profile/index/<?=$review->userName?>"><?= $review->userName ?></a></p>
			<p class="grid_3"><a href="<?=base_url()?>game/index/<?=$review->idGame?>"><?= $review->titleEng ?></a></p>
			<p class="grid_3"><?= $review->title ?></p>
			<div class="grid_1">
				<?php if($review->approved):?>
					<p>Yes</p>
				<?php else:?>
					<form action="<?=base_url()?>reviews_dashboard/approveReview" method="post">
			...
			<div class="grid_1">
				<form action="<?=base_url()?>reviews_dashboard/deleteReview" method="post">
					<input type="hidden" name="idUser" value="<?=$review->idUser?>"/>
					<input type="hidden" name="idTrack" value="<?=$review->idTrack?>"/>
	...
	<script>
		$(function() {
			$('#unapprovedonly').change(function() {
				if(this.checked)
					window.location.href = '<?=base_url()?>reviews_dashboard/index/1';
				else
					window.location.href = '<?=base_url()?>reviews_dashboard/index/0';
			});
		});
		function showReviewDialog(idUser, idTrack) {
			$.getJSON(
				'<?=base_url()?>reviews_dashboard/getReviewForEdit/' + idUser + '/' + idTrack,
				function(review) {
					$('#review-dialog-by').html('<a target="_blank" href="<?=base_url()?>user_profile/index/' + review.userName + '">Author: ' + review.userName + '</a>');
					$('#review-dialog-gameandtrack').html('<a target="_blank" href="<?=base_url()?>game/index/' + review.idGame + '">For: ' + review.titleEng + ' - ' + review.title + '</a>');
		...
		function saveReview(idUser, idTrack) {
			$.post(
				'<?=base_url()?>reviews_dashboard/editReview',
				{
					idUser: idUser,
					idTrack: idTrack,
