...
<?php else: ?>
	...
	<?php $b = TRUE; foreach($trackRequests as $request): //Tracks ?>
		<div style="<?php if($b = !$b): ?> background-color: #dddddd;<?php endif; ?>" class="container_12">
			<div class="grid_1">
				<p><a href="<?=base_url()?>user_profile/index/<?=$request->userName?>"><?= $request->userName ?></a></p>
			</div>
			<div class="grid_2">
				<p><a href="<?=base_url()?>game/index/<?=$request->idGame?>" target="_blank"><?= $request->titleEng ?></a></p>
			</div>
			<div class="grid_2">
				<p><a href="<?=base_url()?>game/index/<?=$request->idGame?>/<?=$request->idTrack?>" target="_blank"><?= $request->title ?></a></p>
			</div>
			<div class="grid_1">
			...
			<div class="grid_1">
				<?= form_open(base_url() . 'screenshot_request_dashboard/deleteTrack', array('id' => 'delete_' . $request->idTrack . $request->idUserRequester)) ?>
	...
	<?php foreach($gameRequests as $request): //Games ?>
		<div style="<?php if($b = !$b): ?> background-color: #dddddd;<?php endif; ?>" class="container_12">
			...
			<div class="grid_1">
				<?= form_open(base_url() . 'screenshot_request_dashboard/deleteGame', array('id' => 'delete_' . $request->idGame . $request->idUserRequester)) ?>
					<input type="hidden" name="idUser" value="<?= $request->idUserRequester ?>" />
