...
<?php $menu = ''; if(count($games) > 0)
	for($p = 1; $p <= $nbPages; $p++)
		if($p == $currentPage)
			...
			$menu .= '<a href="' . base_url() . 'games_dashboard/index/' . $p . '">' . $p . '</a>&nbsp;&nbsp;';
?>
...
<?php /*TODO if(form_error('game_titleeng') or form_error('game_titlejap') or form_error('game_rsn')): ?>
...
<div class="container_16">
	<?= form_open(base_url() . 'games_dashboard/add', array('id' => 'addGame')) ?>
		<div class="grid_4 columnheader">
...
<?php $b = FALSE; foreach($games as $game): ?>
	<?php /*TODO if(form_error("game_{$game->idGame}_titleeng") or form_error("game_{$game->idGame}_titlejap") or form_error("game_{$game->idGame}_rsn")): ?>
	...
	<?php endif; */?>
	<div style="<?php if($b = !$b): ?> background-color: #dddddd;<?php endif; ?>" class="container_16">
		<?= form_open(base_url() . "games_dashboard/update/$currentPage", array('id' => 'update_' . $game->idGame)) ?>
		...
		<div class="grid_1">
			<?= form_open(base_url() . "games_dashboard/delete/$currentPage", array('id' => "delete_{$game->idGame}")) ?>
				<input type="hidden" name="id" value="<?=$game->idGame?>" />
				<div class="btn btn-xs btn-danger" onclick="confirmDelete(<?=$game->idGame?>);">Delete</div>
			</form>
		</div>
		<div class="grid_1">
			<a href="<?=base_url()?>tracks_dashboard/index/<?=$game->idGame?>">Tracks</a>
		</div>
	</div>
<?php endforeach; ?>
