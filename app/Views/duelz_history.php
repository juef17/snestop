...
<?php $b = TRUE; foreach($duelResults as $result): ?>
	<div style="<?php if($b = !$b): ?> background-color: #dddddd;<?php endif; ?>" class="container_16">
		<div class="grid_6">
			<p><a href="<?=base_url()?>game/index/<?=$result->idGameWon?>/<?=$result->idTrackWon?>"><?=$result->gameTitleWon?> - <?=$result->trackTitleWon?></a></p>
		</div>
		<div class="grid_6">
			<p><a href="<?=base_url()?>game/index/<?=$result->idGameLost?>/<?=$result->idTrackLost?>"><?=$result->gameTitleLost?> - <?=$result->trackTitleLost?></a></p>
		</div>
	</div>
<?php endforeach; ?>
