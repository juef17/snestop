...
class Games_Dashboard extends Admin_controller {
	...
	public function add() {
		...
		if(true/*$this->form_validation->run()*/) {
			$this->saveGame();
			return redirect()->to(base_url() . "/index.php/games_dashboard"); //Pour pas réafficher les valeurs soumises.
		} else {
	...
	public function update($page) {
		...
		if($id = $this->request->getVar('id', FILTER_SANITIZE_NUMBER_INT)) {
			...
			if(true/*$this->form_validation->run()*/) {
				$this->updateGame($id);
				return redirect()->to(base_url() . "/index.php/games_dashboard/index/$page");
			} else {
				$data = $this->getUserViewData();
				$data['games'] = $game_model->get_Games($page);
				$data['nbPages'] = $game_model->get_nb_pages();
