<?php
class Rating_Community_model extends CI_Model {
	...
	public function get_Rating_Community($idCommunity = FALSE, $idTrack = FALSE) {
		$this->db->join('Community', 'RatingCommunity.idCommunity = Community.idCommunity', 'inner');
		$this->db->join('Track', 'RatingCommunity.idTrack = Track.idTrack', 'inner');
		if ($idCommunity === FALSE && $idTrack === FALSE) { // on n'a rien
			$query = $this->db->get('RatingCommunity');
			return $query->result_array();
		} else { // on a les deux, yay!
			$this->db->where('RatingCommunity.idCommunity', $idCommunity);
			$this->db->where('RatingCommunity.idTrack', $idTrack);
			$query = $this->db->get('RatingCommunity');
			return $query->row();
		}
	}
	...
	public function new_Rating_Community($idCommunity, $idTrack) {
		...
		return $this->db->insert('RatingCommunity', $data);
	}

	public function delete_Rating_Community($idCommunity, $idTrack) {
		return $this->db->delete('RatingCommunity', array('idCommunity' => $idCommunity, 'idTrack' => $idTrack));
	}

	public function update_ratings($idTrackWinner, $idTrackLoser, $idCommunity) {
		$queryWinner = $this->db->get_where('RatingCommunity', array('idTrack' => $idTrackWinner, 'idCommunity' => $idCommunity));
		$queryLoser = $this->db->get_where('RatingCommunity', array('idTrack' => $idTrackLoser, 'idCommunity' => $idCommunity));
		...
		$this->db->update('RatingCommunity', $dataWinner, array('idTrack' => $idTrackWinner, 'idCommunity' => $idCommunity));
		return $this->db->update('RatingCommunity', $dataLoser, array('idTrack' => $idTrackLoser, 'idCommunity' => $idCommunity));
	}
	...
	public function get_number_of_duels_Track($idTrack, $idCommunity) {
		...
		$this->db->join('User', 'DuelResult.idUser = User.idUser');
		$this->db->where($where);
